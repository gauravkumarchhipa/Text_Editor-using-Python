...
main_application.title('Gaurav text editor')
...
def changed(event=None):
	...
	if text_editor.edit_modified():
		...
		characters = len(text_editor.get(1.0, 'end-1 c').replace(' ', ''))
...
def save_file(event=None):
	...
	try:
		if url:
			...
			contents2 =text_editor.get(1.0, tk.END)
			url.write(content2)
			url.close()
	except:
			return
	...
	def find():
		...
		if word:
			...
			while True:
				...
				matches += 1
				start_pos = end_pos
				text_editor.tag_config('match', foreground='yellow', background='yellow')
